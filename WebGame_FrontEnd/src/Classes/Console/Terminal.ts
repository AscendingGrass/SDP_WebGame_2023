...
export class Terminal{
    ...
    public checkVariableName(name:string):string{
        const variableRegex:RegExp = /^[a-zA-Z_][a-zA-Z0-9_]*$/
        if(variableRegex.test(name) && !this.constGlobalVariables.has(name)) return name
    ...
    public getVariable(variableName:string):Wrapper{
        ...
        const constant = this.constGlobalVariables.get(variableName)
                ...
                case "if" : {
                    ...
                    if(conditionTokens.length == 1){
                        try{
                            ...
                            condition = this.checkVariableName(conditionTokens[0])
                        }
                    }
                    else{
    ...
    private compileSingleCommand(terminal:Terminal, commandTokens:string[]):SingleCommand{
        ...
        if(assignment){
            if(this.checkVariableName(commandTokens[0])){
