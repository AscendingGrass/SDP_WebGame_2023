...
const Help = () => {
  ...
  const handleAsk = async () => {
    console.log("object0");
    if(isLoading.current || !ref.current) return
    console.log("object1");
    isLoading.current = true
    refRes.current.value = "Loading..."
    const body = {
      "prompt": ref.current.value,
    }
    console.log("object2");
    const result = (await axios.post(`${import.meta.env.VITE_BACKEND_URL}/help`, body)).data;
    if(result.response){
      console.log(result.response.parts[0].text);
      refRes.current.value = result.response.parts[0].text
    }
  ...
  return (
    <div className='flex w-screen bg-gray-200 '>
      <div className=" w-2/3 h-[800px]">

        <div className="text-center overflow-y-scroll h-full p-28 ">
      ...
      <div className='w-1/3 p-10'>
        ...
        <textarea ref={ref} className='rounded w-full h-40 resize-none p-2 mt-3' >

        </textarea>
        <button  onClick={handleAsk} className='bg-blue-500 text-white px-12 py-2 rounded mt-3 hover:bg-blue-700 active:bg-blue-500'>
          Ask
        </button>

        <textarea ref={refRes} readOnly className='rounded w-full h-96 resize-none p-2 mt-5 outline-none bg-gray-300' >
